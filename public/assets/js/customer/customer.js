$(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                response.data.data.forEach(function (row) {
                    ...
                    table_html += '<td data-label="Name" data-toggle="tooltip" data-placement="top" title="' + row.full_name + '"> <p><span class="status ' + online_status + '"></span>' + row.full_name + '</p></td>';
                    table_html += '<td data-label="Type" style=" text-transform: capitalize;"> ' + row.account_type + '</td>';
                    table_html += '<td data-label="City" data-toggle="tooltip" data-placement="top" title="' + row.city + '"><p>' + row.city + '</p></td>';
                    table_html += '<td data-label="Mobile" data-toggle="tooltip" data-placement="top" title="' + row.mobile + '"> <p>(+91) ' + row.mobile + '</p></td>';
                    table_html += '<td data-label="Email" data-toggle="tooltip" data-placement="top" title="' + row.email + '"><p>' + row.email + '</p></td>';
                    table_html += '<td data-label="Verification"><button style=" text-transform: capitalize;" class="button-all ' + row.status.toLowerCase() + '" data-toggle="tooltip" data-bs-html="true"  data-placement="top" title="<ul>' + listContent + '</ul>">' + row.status + '</button></td>';
                    table_html += '</tr>';
    ...
    $('#type').on("change", function () {
        ...
        if (type.length == 0) {
            $("#verifytype .SumoSelect").removeClass('filter_color');
        } else {
            $("#verifytype .SumoSelect").addClass('filter_color');
        }
        page = 1;
        myfunction(per_page, page, sort, type, city, login, status, search, search_key, field_sort, percentage);
    });
    $('#city_data').on("change", function () {
        ...
        if (city.length == 0) {
            $("#verifycity .SumoSelect").removeClass('filter_color');
        } else {
            $("#verifycity .SumoSelect").addClass('filter_color');
        }
        page = 1;
        myfunction(per_page, page, sort, type, city, login, status, search, search_key, field_sort, percentage);
    });
    $('#login_status').on("change", function () {
        ...
        if (login.length == 0) {
            $("#verifylogin .SumoSelect").removeClass('filter_color');
        } else {
            $("#verifylogin .SumoSelect").addClass('filter_color');
        }
        page = 1;
        myfunction(per_page, page, sort, type, city, login, status, search, search_key, field_sort, percentage);
    });
