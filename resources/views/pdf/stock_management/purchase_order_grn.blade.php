    ...
    <div class="wrapper">
        ...
        <div class="content">
            <table class="po-details" width="100%;font-size:15px;">
                <tr>
                    <td style="color: #cfcfcf;">PO No:</td>
                    <td>{{ $data['purchase_order_no'] ?? "-" }}</td>
                    <td style="color: #cfcfcf;">created_by:</td>
                    <td>{{ $data['created_by'] ?? '-' }}</td>
                </tr>
                <tr style="padding-top: 50px;">
                    <td style="color: #cfcfcf;">GRN No:</td>
                    <td>{{ $data['grn_no'] ?? "-"  }}</td>
                    <td style="color: #cfcfcf;">Sender Name:</td>
                    <td>{{ $data['manufacturer_name'] ?? '-' }}</td>
                </tr>
                <tr style="padding-top: 50px;">
                    <td style="color: #cfcfcf;">GRN Date:</td>
                    <td>{{ $data['order_date'] ?? "-" }}</td>
                    <td style="color: #cfcfcf;">Transporter Name:</td>
                    <td>{{ $data['transporter_name'] ?? '-' }}</td>
                </tr>
                <tr style="padding-top: 50px;">
                    <td style="color: #cfcfcf;">GRN Location:</td>
                    <td>{{ $data['grn_location'] ?? "-" }}</td>
                ...
                <tbody style="padding-top: 30px !important;">
                    @foreach( $data['products'] as $product )
                        <tr style="border-bottom: 1px solid #cfcfcf;">
                            <td style="padding-left:25px;padding-top:7px;padding-bottom:7px;">{{ $product['name'] }}</td>
                            <td style="padding-left:25px;">{{ $product['quantity'] }}</td>
                        </tr>
                    @endforeach
