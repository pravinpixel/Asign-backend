    ...
    <div class="pages purchase-orders">
        ...
        <div class="section-filter filter-index">
            <div class="filter-setup">
                ...
                <div class="dropbar-bar" @if (!access()->hasAccess('stock-check.create'))  data-toggle="tooltip"
                     data-placement="bottom" title="You don’t have Create permission"
                    @endif >
                    <a href="{{url('stock-check/add')}}" type="button" class="btn apply-btn"
                       @if (!access()->hasAccess('stock-check.create')) disabled
                        @endif >
                        Check Stock
                    </a>
                </div>
        ...
        <section class="table-content">
            <table class="asign-table purchase-order-table">
                <thead>
                <tr>
                    <th scope="col" class="has_sort" data-value="request_id" width="20%">CHECK NO.</th>
                    <th scope="col" class="has_sort" data-value="date" width="20%">CHECK DATE</th>
                    <th scope="col" class="has_sort" data-value="type" width="20%">CHECK TYPE</th>
                    <th scope="col" class="has_sort" data-value="name" width="20%">CHECK DETAILS</th>
                    <th scope="col" class="has_sort" data-value="status" width="20%">STATUS</th>
                </tr>
                </thead>
                <tbody id="tableCtr" class="tbody-detail">

                @include('pages.stock-check.table')
...
@push('scripts')
    ...
    <script>
        ...
        var hasStockCheckEditAccess = {{ access()->hasAccess('stock-check.edit') ? 'true' : 'false' }};
        ...
        $(document).on('click', '.row-class', function (e) {
            ...
            let url = baseUrl + "/stock-check/" + id;
            if (hasStockCheckEditAccess) {
...
<script>
   $('#export_button').click(function() {
    ...
    $.ajax({
        url: baseUrl + "/stock-check/export",
        type: 'GET',
        data: {
            search: search,
        },
