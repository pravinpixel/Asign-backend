...
<div class="pages purchase-order-summary">
    <section class="m-header">
        <main class="hstack gap-3">
            <a href="{{ url('/goods-received-notes') }}">
                <img src="{{ asset('icons/arrow-left-alt.svg') }}" width="24" height="24" class="cP" />
            </a>
            <h4>{{ $grnDetails->grn_no ?? "-" }}</h4>
        </main>
        <div>
            @php
                $allSegments = request()->segments();
                $grnId = $allSegments[count($allSegments)-2];
                $type = end($allSegments);
            @endphp
            <button type="button" class="btn apply-btn grn_print_btn" style="font-color:white;">
                Print
            </button>
        </div>
    </section>
    <section class="section-inner">
        ...
        <ul class="personal-info">
            <li>
                <span>PO/STO No.</span>
                @if ($type === "sto")
                    <span>{{ $grnDetails->sto_no ?? "-" }}</span>
                @else
                    <span>{{ $grnDetails->po_no ?? "-" }}</span>
                @endif
            </li>
            <li>
                <span>GRN No.</span>
                <span>{{ $grnDetails->grn_no ?? "-"  }}</span>
            </li>
            <li>
                <span>GRN Date</span>
                <span>{{ $grnDetails->created_on ?? "-" }}</span>
            </li>
            <li>
                <span>GRN Location</span>
                <span>{{ $grnDetails->branch_location->location ?? "-" }}</span>
            </li>
            <li>
                <span>Created by</span>
                <span>{{ $grnDetails->user_name ?? '-' }}</span>
            </li>
        </ul>
        ...
        <ul class="personal-info">
            <li>
                ...
                @if ($type === "sto")
                    <span>{{ $grnDetails->sto_sender_name->location ?? '-' }}</span>
                @else
                    <span>{{ $grnDetails->manufacturer->name ?? '-' }}</span>
                @endif
            </li>
            <li>
                <span>Transporter Name</span>
                <span>{{ $grnDetails->transporter->name ?? '-' }}</span>
            </li>
        </ul>
    </section>
    <section class="table-content">
        @include( 'pages.stock_management.grn.grn_summary_table' )
    </section>
</div>
@endsection
@push('scripts')
...
<script type="text/javascript">
    ...
    var printUrl = "{{ route( 'grn.pdf', [ "id" => $grnId, "type" => $type ] ) }}";
