...
<div class="pages purchase-orders">
    <section class="main-header filterHeaderCtr">
        <div class="section-breadcrumb">
            <ul>
                <li>
                    <img src="{{ asset('icons/clipboard-text.png') }}" width="18" height="18" /><a
                        href="#">Label Management</a>
                </li>
                <li>/</li>
                <li>
                    <a href="{{ url('goods-received-notes') }}">Good Received Notes</a>
                </li>
                <li></li>
            </ul>
        </div>
    ...
    @include('pages.stock_management.grn.grn_filter')
    <section class="table-content">
        <table class="asign-table purchase-order-table" id="grn_list_table">
            <thead>

            <tr>
                <th scope="col" class="has_sort" data-value="grn_no"  width="15%">GRN NO.</th>
                <th scope="col" class="has_sort" data-value="order_no"  width="15%">ORDER NO.</th>
                <th scope="col" class="has_sort" data-value="sender"  width="15%">SENDER</th>
                <th scope="col" class="has_sort" data-value="branch_name"  width="20%">GRN LOCATION</th>
                <th scope="col" class="has_sort" data-value="created_on"  width="15%">CREATED ON</th>
            </tr>
            </thead>
            <tbody id="tableCtr" class="tbody-detail">
        @include('pages.stock_management.grn.grn_table')
    </tbody>
</table>
...
<script src="{{ asset('plugins/lodash/lodash.min.js') }}"></script>
    ...
    <script type="text/javascript">
        ...
        var grmAccess = {{ access()->hasAccess('goods-received-note.edit') ? 'true' : 'false' }};
        ...
        $(document).on('click', '.row-class td:not(.disabled-td)', function(e) {
            ...
            let orderNo = row.find('td').eq(1).text();
            if(grmAccess){
                if (orderNo.startsWith("STO")) {
                window.location.href = "{{ url('/goods-received-notes/summary') }}/" + id + "/sto";
            } else {
                window.location.href = "{{ url('/goods-received-notes/summary') }}/" + id + "/po";
            }
            }
    ...
    <script type="text/javascript">
        ...
        config.links.list = "{{ route( 'grn.list' ) }}";
