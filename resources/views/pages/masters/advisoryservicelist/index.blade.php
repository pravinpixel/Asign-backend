...
@section('title', 'Advisory Service List')
@section('style')
<style type="text/css">
  a[disabled] {
    pointer-events: none;
...
<section class="main-header">
    <div class="section-breadcrumb">
            ...
            <li>
                <a href="{{url('masters/advisory-service')}}">Advisory Service List</a>
            </li>
            <li>
            </li>
        </ul>
     ...
     <div class="section-title">
        <h4>Advisory Service List<span id="total"></span></h4>
    </div>
    <!-- #....Header -->
<!-- #...search -->
<!-- #...end search -->
  ...
  <div class="filter-setup">
    <div class="search-bar">
      <div class="input-group filter-with">
        ...
        <ul class="dropdown-menu width-ul">
          ...
          <li class="li-one  padding-li">
            <div class="custom-check">
              <input class="form-check-input" type="radio" name="flexRadioDefault1" id="cost" value="cost">
              <label class="radio-label font-drop" for="cost">
                Cost
              </label>
            </div>
          </li>
    ...
    <div class="dropbar-bar" @if (!access()->hasAccess('master.create'))  data-toggle="tooltip" data-placement="bottom" title="You don’t have Create permission" 
      ...
      <button type="button" class="btn cancel-btn" id="add" onclick="getModal()"  @if (!access()->hasAccess('master.create')) disabled @endif>
        <img src="{{ asset('icons/add.png') }}">  Add Advisory Service List
        </button>
    </div>
  </div>
        ...
        <thead>
            <tr>
              <th scope="col" width="20%" >SR NO.</th>
                <th scope="col" width="15%" class="has_sort" data-value="title">Title</th>
                <th scope="col" width="15%" class="has_sort" data-value="code">Code</th>
                <th scope="col" width="15%" class="has_sort" data-value="cost">Cost</th>
...
<script type="text/javascript">
  var url="{{ url('masters/advisory-service/list') }}";
  var edit="{{ asset('icons/edit-2.png') }}";
  var edit_model="{{ route('advisory-service.add_edit') }}";
    var save_model = "{{ route('advisory-service.save') }}";
    var check="{{ route('advisory-service.check') }}";
    var delete_icon = "{{ asset('icons/image_2024_01_24T04_03_24_643Z.png') }}";
  var delete_url="{{ route('advisory-service.delete') }}";
</script>
 <script type="text/javascript" src="{{ asset('js/masters/advisoryservice.js') }}"></script>
@endpush
