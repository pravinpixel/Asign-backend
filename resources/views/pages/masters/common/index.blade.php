...
<section class="main-header">
  <div class="section-breadcrumb">
    <ul>
      ...
      <li>
      <a href="#">{{$data['breadcrumbs']}}</a>
      </li>
      <li>
      /
      </li>
      <li>
      <a href="{{ url($routePrefix) }}">{{ $data['title'] }}</a>
     </li>
  ...
  <div class="section-title">
    <h4>{{$data['title']}}<span id="total"></span></h4>
  </div>
  <!-- #....Header -->
  <!-- #...search -->
      ...
      @endif>
        ...
        <button type="button" class="btn cancel-btn" id="add" onclick="getModal()"  
         @if (!access()->hasAccess('master.create')) disabled @endif>
          <img src="{{ asset('icons/add.png') }}"> Add {{$data['title']}}
        </button>
...
<script type="text/javascript">
  var routeName = "{{ request()->route()->getPrefix() }}";
  var dataTitle = "{{$data['title']}}";
  var moduleData;
  if (dataTitle == 'Medium') {
    moduleData = 'mediums'; 
  }else if(dataTitle == 'Surface Medium'){
    moduleData = 'surface mediums'; 
  } else {
    moduleData ="{{ strtolower(Str::plural($data['title'])) }}";
  }
  var url = "{{ url('') }}/" + routeName + "/list";
  var edit = "{{ asset('icons/edit-2.png') }}";
  var delete_icon = "{{ asset('icons/image_2024_01_24T04_03_24_643Z.png') }}";
  var edit_model = "{{ url('') }}/"+routeName+"/edit";
  var delete_url = "{{ url('') }}/"+routeName+"/delete";
  var save_model = "{{ url('') }}/"+routeName+"/save";
  var check = "{{ url('') }}/"+routeName+"/check";
</script>
<script type="text/javascript" src="{{ asset('js/masters/common.js') }}"></script>
@endpush