...
<section class="main-header">
    <div class="section-breadcrumb">
            ...
            <li>
                <a href="{{url('masters/currency')}}">Currency</a>
            </li>
            <li>
            </li>
        </ul>
     ...
     <div class="section-title">
        <h4>Currency<span id="total"></span></h4>
    </div>
    <!-- #....Header -->
<!-- #...search -->
  ...
  <div class="filter-setup">
    ...
    <div class="dropbar-bar"  @if (!access()->hasAccess('master.create'))  data-toggle="tooltip" data-placement="bottom" title="You don’t have Create permission"
      ...
      <button type="button" class="btn cancel-btn" id="add" onclick="getModal()" @if (!access()->hasAccess('master.create')) disabled @endif>
        <img src="{{ asset('icons/add.png') }}">         Add Currency
        </button>
    </div>
  </div>
...
<section class="table-content">
    <table class="asign-table customer-table customer" id="master_table">
        <thead>
            <tr>
              <th scope="col" width="20%" class="" id="#" >SR NO.</th>
...
<script type="text/javascript">
  var url="{{ url('masters/currency/list') }}";
  var edit="{{ asset('icons/edit-2.png') }}";
  var edit_model="{{ route('currency.add_edit') }}";
  var save_model="{{ route('currency.save') }}";
  var check="{{ route('currency.check') }}";
  var delete_icon = "{{ asset('icons/image_2024_01_24T04_03_24_643Z.png') }}";
  var delete_url="{{ route('currency.delete') }}";
</script>
 <script type="text/javascript" src="{{ asset('js/masters/currency.js') }}"></script>
@endpush
