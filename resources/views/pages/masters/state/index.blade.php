...
<section class="main-header">
    <div class="section-breadcrumb">
          ...
          <li>
            ...
            <li>
                <a href="{{url('masters/state')}}">State</a>
            </li>
            <li>
            </li>
        </ul>
     ...
     <div class="section-title">
        <h4>State <span id="total"></span></h4>
    </div>
    <!-- #....Header -->
<!-- #...search -->
  ...
  <div class="filter-setup">
    ...
    <div class="dropbar-bar" @if (!access()->hasAccess('master.create'))  data-toggle="tooltip" data-placement="bottom" title="You don’t have Create permission" 
      ...
      <button type="button" class="btn cancel-btn" id="add" onclick="getModal()" @if (!access()->hasAccess('master.create')) disabled @endif>
        <img src="{{ asset('icons/add.png') }}">  Add State
        </button>
    </div>
...
<script type="text/javascript">
  var url="{{ url('masters/state/list') }}";
  var edit="{{ asset('icons/edit-2.png') }}";
  var edit_model="{{ route('state.add_edit') }}";
    var save_model = "{{ route('state.save') }}";
    var check="{{ route('state.check') }}";
    var delete_icon = "{{ asset('icons/image_2024_01_24T04_03_24_643Z.png') }}";
  var delete_url="{{ route('state.delete') }}";
</script>
 <script type="text/javascript" src="{{ asset('js/masters/state.js') }}"></script>
@endpush
