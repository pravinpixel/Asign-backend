...
class ArtistController extends Controller
{
    ...
    {
        $city = Customer::where('account_type', 'artist')->select('city')->whereNotNull('city')->groupBy('city')->get();
        return view('pages.customers.artist.index', [
            'cities' => $city
        ]);
    }
    ...
    public function get(Request $request)
    {
        ...
        try {
            ...
            $query = Customer::where("mobile", '<>', '')->where('account_type', 'artist');
            $query->select('id', 'is_represent_contract','aa_no', 'full_name', 'display_name', 'email', 'mobile', 'city', 'status', 'account_type', 'is_mobile_verified', 'is_email_verified', 'is_accept_terms', 'aadhaar_no', 'pan_no', 'cin_no', 'gst_no', 'is_pan_verify', 'is_aadhaar_verify', 'profile_completion', UtilsHelper::checkOnline());
    ...
    public function view(Request $request, $id)
    {
        ...
        $data = Customer::with('galleries.gallery')->find($id);
        if(!$data){
            return redirect()->route('customer.artist.index');
        }
        ...
        return view('pages.customers.artist.view', [
            'data' => $data,
            'titles' => [],
            'locations' => [],
            'objects' => $objects,
    ...
    public function delete($id)
    {
        try {
            ...
            return $this->returnSuccess([], "Customer deleted successfully");
        } catch (\Exception $e) {
            return $this->returnError($e->getMessage());
        }
    }
    ...
    public function export(Request $request)
    {
        try {
            ...
            $query->where('account_type', 'artist');
            ...
            return Excel::download(new ArtistExport($customers), 'customers.xlsx');
        } catch (\Exception $e) {
            return $this->returnError($e->getMessage());
        }
    }

    public function update(Request $request, $customer_artist_id)
    {
        try {
            if ($request->type === 'rejected') {
                CustomerArtist::where('customer_artist_id', $customer_artist_id)
                    ->update([
                        'is_verified' => 0,
            ...
            $empty_representation = CustomerArtist::where('customer_id', $request->customer_id)
            ...
            $activityLogs = CustomerActivityLog::with('customer', 'user')
                ...
                ->get();
            $customerArtists = CustomerArtist::with('artist')->where('customer_id', $request->customer_id)->orderBy('id', 'desc')->get();
